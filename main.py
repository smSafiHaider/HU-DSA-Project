...
class admin:
    ...
    def customCheck(self):
        ...
        def hashing(string): return md5(string.encode()).hexdigest()
...
class booking:
    ...
    def book(self):
        ...
        time = input(
            'Enter the renting Date and Time (yyyy, dd, mm, hr, mn): ').split(", ")
        ...
        pdf.cell(
            100, 10, txt=f"Pick-up Time: {month_[int(time[2])]} {time[1]}, {time[0]} | {time[3]}:{time[4]}:00", ln=9, align='L')
        pdf.cell(100, 10, txt=f"Renting Span: {rent_span}", ln=10, align='L')
        pdf.cell(
            100, 10, txt=f"Allowed Running: {distance} km", ln=11, align='L')
        pdf.cell(
            100, 10, txt=f"Total Charge: {int(distance) * int(costPerKm)}", ln=12, align='L')

        pdf.set_font("Arial", size=6)
        pdf.cell(
            100, 10, txt="Late delivery of car may result in additional charges.", ln=13, align='L')

        pdf.output(
            f"D:/2nd semester/DSA/project/{NIC}_booking_details.pdf", 'F')
